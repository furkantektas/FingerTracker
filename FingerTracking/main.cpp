...
//    Mat img = Mat::zeros( img.size(), CV_8UC3 );

    int handInd = findLargestContour(contours);
    Rect handRect = boundingRect(contours[handInd]);
    filterConvexes(convdefect[handInd], contours[handInd], handRect);

    
    // drawing convex hull, bounding rectangle and convexity defect
    
//    drawContours( drawing, hull, handInd, green, 3, 8, vector<Vec4i>(), 0, Point() );
//    rectangle( drawing, handRect.tl(), handRect.br(), red, 2, 8, 0 );
//      circle( drawing, center[i], (int)radius[i], blue, 2, 8, 0 );
//    drawConvexity(drawing, fingers, contours[handInd]);
    printFingerCount(rawFrame, fingers.size());
    drawFingerLines(rawFrame);
    
//    cout << "BRect " << handRect.width<<"x"<<handRect.height<<endl;

//    drawContours(drawing, hull, handInd, red,3,8,vector<Vec4i>(),0,Point());
    drawConvexity(rawFrame, convdefect[handInd], contours[handInd]);
